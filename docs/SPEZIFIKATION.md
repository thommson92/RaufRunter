...
src/profile-ui.js        Avatare, Foto-Laden/-Zuschneiden, Combobox-Markup
src/crop-model.js        Zuschnitt-Geometrie (Pan/Zoom, rein, testbar)
src/photo-crop.js        Zuschnitt-Widget (Pointer-Events/Pinch/Zoom-Regler)
...
| **M18** | **Profilfoto-Zuschnitt**: Foto-Upload schnitt bisher immer stumpf die Bildmitte quadratisch zu (`photoToDataUrl`) — bei Hochformat-Handyfotos landete damit oft der falsche Bereich im Avatar. Neuer Zwischenschritt zwischen Dateiauswahl und Speichern: Ausschnitt & Zoom per Ziehen (Pointer Events), Kneifen (zwei Finger) oder Regler wählen, dann „Übernehmen"/„Abbrechen" — analog zur Geber-Auslosung ein eigener Bildschirm statt Modal (`ui.cropper`, `renderPhotoCropView`). Geometrie in `src/crop-model.js` (rein, testbar: `coverScale`/`clampView`/`sourceRect`/`zoomAround`/`panBy`), Widget in `src/photo-crop.js`. `profile-ui.js`s `photoToDataUrl` ist dafür in `loadPhotoSource` (Datei laden, auf 1024px verkleinern) und `cropToDataUrl` (gewählten Ausschnitt auf 256px rendern) aufgeteilt. | ✅ fertig |
